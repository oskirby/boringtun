...
pub struct Timers {
    ...
    current: Duration,
    timers: [AtomicU64; TimerName::Top as usize],
...
impl Timers {
    pub(super) fn new(persistent_keepalive: Option<u16>, reset_rr: bool) -> Timers {
        Timers {
            ...
            want_keepalive: AtomicBool::new(false),
    ...
    pub(super) fn clear(&mut self) {
        ...
        let msec = now.as_millis() as u64;
        for t in &mut self.timers[..] {
            t.store(msec, Ordering::Release);
        }
        self.want_handshake.store(false, Ordering::Release);
        self.want_keepalive.store(false, Ordering::Release);
    }
}
...
impl Tunn {
    pub(super) fn timer_tick(&self, timer_name: TimerName) {
        match timer_name {
            TimeLastPacketReceived => {
                self.timers.want_keepalive.store(true, Ordering::Release);
                self.timers.want_handshake.store(false, Ordering::Release);
            }
            TimeLastPacketSent => {
                self.timers.want_handshake.store(true, Ordering::Release);
                self.timers.want_keepalive.store(false, Ordering::Release);
            }
            _ => {}
        }
    ...
    pub(super) fn timer_fetch(&self, timer_name: TimerName) -> Duration {
        Duration::from_millis(self.timers.timers[timer_name as usize].load(Ordering::Acquire))
    }
    ...
    pub(super) fn timer_tick_session_established(
        ...
        self.timers.session_timers[session_idx % crate::noise::N_SESSIONS] = self.timers.current;
    ...
    pub fn update_timers<'a>(&mut self, dst: &'a mut [u8]) -> TunnResult<'a> {
        ...
        self.timers.current = now;

        self.update_session_timers(now);

        // Load timers only once:
        let session_established = self.timer_fetch(TimeSessionEstablished);
        let handshake_started = self.timer_fetch(TimeLastHandshakeStarted);
        let aut_packet_received = self.timer_fetch(TimeLastPacketReceived);
        let aut_packet_sent = self.timer_fetch(TimeLastPacketSent);
        let data_packet_received = self.timer_fetch(TimeLastDataPacketReceived);
        let data_packet_sent = self.timer_fetch(TimeLastDataPacketSent);
...

        {
            ...
            if let Some(time_init_sent) = self.handshake.timer() {
                ...
                if data_packet_sent > aut_packet_received
                    ...
                    && self.timers.want_handshake.swap(false, Ordering::AcqRel)
                {
                ...
                if !handshake_initiation_required {
                    ...
                    if data_packet_received > aut_packet_sent
                        ...
                        && self.timers.want_keepalive.swap(false, Ordering::AcqRel)
                    {
                    ...
                    if persistent_keepalive > 0
                        && (now - self.timer_fetch(TimePersistentKeepalive)
    ...
    pub fn time_since_last_handshake(&self) -> Option<Duration> {
        let current_session = self.current.load(Ordering::Relaxed);
        if self.sessions[current_session % super::N_SESSIONS].is_some() {
            ...
            let duration_since_session_established = self.timer_fetch(TimeSessionEstablished);
